...
def _p2_source_evidence(bundle:dict[str,Any])->dict[str,Any]:
    ...
    if bundle["sourceFamily"]=="quick_forecast_p2":
        assignment=bundle["assignment"];forecast=bundle["forecast"]
        return {**common,
            "runRecordPath":"metadata/run.json","runRecordSha256":bundle["commit"]["runRecordSha256"],
            "forecastCalibrationPath":"artifacts/forecast_calibration.json","forecastCalibrationSha256":hashes["forecast_calibration.json"],
            "assessmentReferenceStatus":"not_applicable_no_assessment_reference",
            "assignmentProvenance":{"assignmentId":assignment["assignmentId"],"assignmentCommitSha256":assignment["assignmentCommitSha256"],"assignmentAction":assignment["assignmentAction"],"authoritySnapshotSha256":assignment["authoritySnapshotSha256"],"lifecyclePolicy":{"policyId":assignment["lifecyclePolicyId"],"policyVersion":assignment["lifecyclePolicyVersion"],"policySha256":assignment["lifecyclePolicySha256"]}},
            "forecastPresentationMode":forecast["forecastPresentationMode"],"calibrationStatus":forecast["calibrationStatus"],"uncertaintyReasonCode":forecast["uncertaintyReasonCode"],"deploymentModelAdopted":False}
...
def execute(args:argparse.Namespace)->dict[str,Any]:
    ...
    if schema_version in {"2.0","2.1"}:observation.update({"sourceFamily":bundle["sourceFamily"],"monitoringPolicy":{"policyId":policy["policy_id"],"policyVersion":policy["policy_version"],"policySha256":digest}})
    ...
    _update_job(job_path,job,"evaluating_forecast_outcome");metrics=evaluate_outcome(forecast["forecastRaw"],submitted["observedRaw"],bundle["uncertainty"],schema_version in {"2.0","2.1"})
    if schema_version=="1.0":
        ...
        outcome={"schemaVersion":schema_version,"outcomeId":job["outcomeId"],"jobId":job["jobId"],"monitoringPolicy":{"policyId":policy["policy_id"],"policyVersion":policy["policy_version"],"policySha256":digest},"sourceFamily":bundle["sourceFamily"],"sourceForecastRunId":job["forecastRunId"],"sourceForecastCommitPath":f"runs/{job['forecastRunId']}/metadata/commit.json","sourceForecastCommitSha256":job["expectedForecastCommitSha256"],"sourceForecastSchemaVersion":bundle["commit"]["schemaVersion"],"sourceEvidence":_p2_source_evidence(bundle),"observationArtifactPath":"artifacts/observation.json","observationArtifactSha256":sha256_file(staging/"artifacts/observation.json"),"deploymentId":job["deploymentId"],"datasetId":forecast["datasetId"],"geography":geography,"modelId":bundle["modelId"],"modelFamily":bundle["modelFamily"],"modelParametersSha256":bundle["parameterHash"],"preprocessingIdentity":bundle.get("preprocessingIdentity"),"candidateRegistrySha256":bundle["candidateRegistrySha256"],"featureOrderSha256":bundle["featureOrderSha256"],"sourcePolicy":bundle["sourcePolicy"],"targetColumn":forecast["target"],"forecastHorizonWeeks":forecast["horizonWeeks"],"forecastOriginPeriod":bundle["origin"],"forecastTargetPeriod":forecast["targetPeriod"],"forecastRaw":forecast["forecastRaw"],"forecastReported":forecast["forecastReported"],"observedRaw":submitted["observedRaw"],"observationSourceType":submitted["observationSourceType"],"observationSourceId":submitted["observationSourceId"],"observationRecordId":submitted["observationRecordId"],**metrics,"evaluatedAt":evaluated_at,"limitations":[policy["maturity_statement"],"Monitoring is evidence-only and does not classify degradation.","Missing empirical-range calibration is not an integrity failure."]}
        if schema_version=="2.0":outcome.pop("preprocessingIdentity")
    ...
    committed=commit_forecast_outcome(root,staging,job,policy,bundle["snapshot"])
    return committed
