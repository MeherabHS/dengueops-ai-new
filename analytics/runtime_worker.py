...
def ensure_structure(root: Path) -> None:
    for relative in ("jobs/pending", "jobs/running", "jobs/completed", "jobs/failed", "staging", "runs", "assessment-staging", "assessments", "outcome-staging", "forecast-outcomes", "decisions", "assessment-decisions", "authorizations", "authorization-state", "deployments", "locks"):
...
def load_job(path: Path) -> dict[str, Any]:
    ...
    identity_field = "assessmentId" if kind == "dataset_assessment" else "outcomeId" if kind == "forecast_outcome" else "runId"
    fields = ("jobId", identity_field) if kind == "forecast_outcome" else ("jobId", "workspaceId", identity_field)
    for field in fields:
...
def recover_stale_jobs(root: Path) -> None:
    ...
    for path in (root / "jobs" / "running").glob("*.json"):
        try:
            ...
            identity = job["assessmentId"] if kind == "dataset_assessment" else job["outcomeId"] if kind == "forecast_outcome" else job["runId"]
            staging = root / ("assessment-staging" if kind == "dataset_assessment" else "outcome-staging" if kind == "forecast_outcome" else "staging") / identity
            if staging.exists():
...
def execute_claimed(root: Path, job_path: Path, worker_id: str) -> None:
    ...
    kind = job.get("jobKind", "quick_forecast")
    workspace = None if kind in {"approved_forecast", "forecast_outcome"} else require_within(root, root / "workspaces" / job["workspaceId"], "workspace")
    identity = job["assessmentId"] if kind == "dataset_assessment" else job["outcomeId"] if kind == "forecast_outcome" else job["runId"]
    staging_collection = "assessment-staging" if kind == "dataset_assessment" else "outcome-staging" if kind == "forecast_outcome" else "staging"
    ...
    input_bytes = 0 if kind == "forecast_outcome" else sum((input_root / "inputs" / group / name).stat().st_size for group, name in (
    ...
    started = now()
    initial_progress = "preparing_assessment" if kind == "dataset_assessment" else "preparing_approved_forecast" if kind == "approved_forecast" else "validating_forecast_commit" if kind == "forecast_outcome" else "preparing_isolated_run"
    ...
    script = "runtime_assessment.py" if kind == "dataset_assessment" else "runtime_approved_forecast.py" if kind == "approved_forecast" else "runtime_forecast_outcome.py" if kind == "forecast_outcome" else "runtime_quick_forecast.py"
    ...
    elif kind != "forecast_outcome": command.extend(["--workspace", str(workspace)])
    ...
    job = update_job(job_path, job, processId=process.pid, progress="validating_forecast_commit" if kind == "forecast_outcome" else "building_features")
    ...
    if timed_out:
        job = update_job(job_path, job, status="timed_out", progress="timed_out", completedAt=now(), processId=None,
            error={"code": "assessment_timeout" if kind == "dataset_assessment" else "approved_forecast_timeout" if kind == "approved_forecast" else "forecast_outcome_timeout" if kind == "forecast_outcome" else "quick_forecast_timeout",
                "message": "The dataset assessment exceeded its execution limit." if kind == "dataset_assessment" else "The approved forecast exceeded its execution limit." if kind == "approved_forecast" else "Forecast outcome evaluation exceeded its execution limit." if kind == "forecast_outcome" else "The Quick Forecast exceeded its execution limit.", "retryable": kind != "approved_forecast"})
    ...
    if process.returncode != 0:
        ...
        job = update_job(job_path, job, status="failed", progress="execution_failed", completedAt=now(), processId=None,
            error={"code": "assessment_failed" if kind == "dataset_assessment" else "approved_forecast_failed" if kind == "approved_forecast" else outcome_code if kind == "forecast_outcome" else "quick_forecast_failed",
                "message": "The isolated dataset assessment did not complete." if kind == "dataset_assessment" else "The approved forecast did not complete." if kind == "approved_forecast" else "Forecast outcome evaluation did not complete." if kind == "forecast_outcome" else "The isolated Quick Forecast did not complete.", "retryable": outcome_retryable if kind == "forecast_outcome" else kind != "approved_forecast"})
        os.replace(job_path, root / "jobs" / "failed" / job_path.name)
        return
    committed_root = root / ("assessments" if kind == "dataset_assessment" else "forecast-outcomes" if kind == "forecast_outcome" else "runs") / identity
    latest = root / "deployments" / job["deploymentId"] / ("monitoring/latest.json" if kind == "forecast_outcome" else "latest.json")
    if not committed_root.exists() or (kind in {"quick_forecast", "approved_forecast", "forecast_outcome"} and not latest.exists()):
        job = update_job(job_path, job, status="failed", progress="commit_failed", completedAt=now(), processId=None,
            error={"code": "assessment_commit_missing" if kind == "dataset_assessment" else "forecast_outcome_commit_missing" if kind == "forecast_outcome" else "runtime_commit_missing",
                "message": "The process exited without a valid immutable commit.", "retryable": True})
        os.replace(job_path, root / "jobs" / "failed" / job_path.name)
        return
    completion = {"committedAssessmentId": job["assessmentId"]} if kind == "dataset_assessment" else {"committedOutcomeId": job["outcomeId"]} if kind == "forecast_outcome" else {"committedRunId": job["runId"]}
