...
def ensure_structure(root: Path) -> None:
    for relative in ("jobs/pending", "jobs/running", "jobs/completed", "jobs/failed", "staging", "runs", "assessment-staging", "assessments", "decisions", "assessment-decisions", "authorizations", "authorization-state", "deployments", "locks"):
...
def load_job(path: Path) -> dict[str, Any]:
    ...
    identity_field = "assessmentId" if kind == "dataset_assessment" else "runId"
    for field in ("jobId", "workspaceId", identity_field):
...
def recover_stale_jobs(root: Path) -> None:
    ...
    for path in (root / "jobs" / "running").glob("*.json"):
        try:
            ...
            identity = job["assessmentId"] if kind == "dataset_assessment" else job["runId"]
            staging = root / ("assessment-staging" if kind == "dataset_assessment" else "staging") / identity
            if staging.exists():
...
def execute_claimed(root: Path, job_path: Path, worker_id: str) -> None:
    ...
    kind = job.get("jobKind", "quick_forecast")
    workspace = None if kind == "approved_forecast" else require_within(root, root / "workspaces" / job["workspaceId"], "workspace")
    identity = job["assessmentId"] if kind == "dataset_assessment" else job["runId"]
    staging_collection = "assessment-staging" if kind == "dataset_assessment" else "staging"
    ...
    input_bytes = sum((input_root / "inputs" / group / name).stat().st_size for group, name in (
    ...
    started = now()
    initial_progress = "preparing_assessment" if kind == "dataset_assessment" else "preparing_approved_forecast" if kind == "approved_forecast" else "preparing_isolated_run"
    ...
    script = "runtime_assessment.py" if kind == "dataset_assessment" else "runtime_approved_forecast.py" if kind == "approved_forecast" else "runtime_quick_forecast.py"
    ...
    else: command.extend(["--workspace", str(workspace)])
    ...
    job = update_job(job_path, job, processId=process.pid, progress="building_features")
    ...
    if timed_out:
        job = update_job(job_path, job, status="timed_out", progress="timed_out", completedAt=now(), processId=None,
            error={"code": "assessment_timeout" if kind == "dataset_assessment" else "approved_forecast_timeout" if kind == "approved_forecast" else "quick_forecast_timeout",
                "message": "The dataset assessment exceeded its execution limit." if kind == "dataset_assessment" else "The approved forecast exceeded its execution limit." if kind == "approved_forecast" else "The Quick Forecast exceeded its execution limit.", "retryable": kind != "approved_forecast"})
    ...
    if process.returncode != 0:
        job = update_job(job_path, job, status="failed", progress="execution_failed", completedAt=now(), processId=None,
            error={"code": "assessment_failed" if kind == "dataset_assessment" else "approved_forecast_failed" if kind == "approved_forecast" else "quick_forecast_failed",
                "message": "The isolated dataset assessment did not complete." if kind == "dataset_assessment" else "The approved forecast did not complete." if kind == "approved_forecast" else "The isolated Quick Forecast did not complete.", "retryable": kind != "approved_forecast"})
        os.replace(job_path, root / "jobs" / "failed" / job_path.name)
        return
    committed_root = root / ("assessments" if kind == "dataset_assessment" else "runs") / identity
    latest = root / "deployments" / job["deploymentId"] / "latest.json"
    if not committed_root.exists() or (kind in {"quick_forecast", "approved_forecast"} and not latest.exists()):
        job = update_job(job_path, job, status="failed", progress="commit_failed", completedAt=now(), processId=None,
            error={"code": "assessment_commit_missing" if kind == "dataset_assessment" else "runtime_commit_missing",
                "message": "The process exited without a valid immutable commit.", "retryable": True})
        os.replace(job_path, root / "jobs" / "failed" / job_path.name)
        return
    completion = {"committedAssessmentId": job["assessmentId"]} if kind == "dataset_assessment" else {"committedRunId": job["runId"]}
