...
POLICY_SHA = "2c132daecd0a0f8579de101a540be52bff063da9cd96600419bd55331f30b9a7"
MONITORING_SHA = "cc3b533b7cfc16b841401b7a60f6308632526f58d5685578d7f368dec96a09d9"
PREVIOUS_POLICY_SHA = "69db63b59f6e0dbbd5d45e98868ce0cafae1a9407d23595c89ec52491b713c98"
PREVIOUS_MONITORING_SHA = "5c3e1f7f14ab6a0a2fbc28639411a0269224b6f71746a315b9c6e159a6eacca6"
...
def load_and_validate_model_degradation_policy(deployment_id: str = "dhaka_south", schema_version: str = "2.0", policy_version: str = POLICY_VERSION, policy_sha256: str | None = None) -> tuple[dict[str, Any], str]:
    ...
    previous=(deployment_id,schema_version,policy_version)==("dhaka_south","2.0","p2-v2")
    ...
    path = ROOT / "config/deployments/dhaka_south" / ("model_degradation_evidence_policy.json" if current else "model_degradation_evidence_policy_p2-v2.json" if previous else "model_degradation_evidence_policy_p2-v1.json")
    ...
    expected_policy_sha=POLICY_SHA if current else PREVIOUS_POLICY_SHA if previous else HISTORICAL_POLICY_SHA
    expected_monitoring = {"policy_id":"RUNTIME.FORECAST_OUTCOME.MONITORING","policy_version":POLICY_VERSION,"policy_sha256":MONITORING_SHA,"schema_version":"2.1"} if current else {"policy_id":"RUNTIME.FORECAST_OUTCOME.MONITORING","policy_version":"p2-v2","policy_sha256":PREVIOUS_MONITORING_SHA,"schema_version":"2.1"} if previous else {"policy_id":"RUNTIME.FORECAST_OUTCOME.MONITORING","policy_version":"p2-v1","policy_sha256":HISTORICAL_MONITORING_SHA}
    ...
    expected_identity=("2.0",POLICY_ID,POLICY_VERSION,"active",deployment_id) if current else ("2.0",POLICY_ID,"p2-v2","active",deployment_id) if previous else ("1.0",POLICY_ID,"p2-v1","active",deployment_id)
    ...
    if (current or previous) and policy.get("accepted_source_families") != ["quick_forecast_p1","quick_forecast_p2","approved_forecast_p1","approved_forecast_p2"]:
