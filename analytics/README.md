...
`runtime_approved_forecast.py` handles only `approved_forecast` jobs created from a committed, hash-bound decision and a reserved one-run authorization. It reads canonical inputs from the immutable assessment, verifies the decision, authorization, assessment, candidate registry, selected parameter hash, and 18-feature contract, then fits the selected governed learned estimator once on all 173 labelled rows. It never reruns candidate comparison, uncertainty calibration, or the operational engine.
