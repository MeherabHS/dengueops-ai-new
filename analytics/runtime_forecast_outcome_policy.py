...
def _policy_path(deployment_id:str,schema_version:str,policy_version:str)->Path:
    ...
    if (schema_version,policy_version)==("2.1",P21_VERSION):return ROOT/"config/deployments"/deployment_id/"forecast_outcome_policy.json"
...
def load_and_validate_forecast_outcome_policy(deployment_id:str,schema_version:str="2.1",policy_version:str|None=None,policy_sha256:str|None=None)->tuple[dict[str,Any],str]:
    expected_version=policy_version or (P1_VERSION if schema_version=="1.0" else P2_VERSION if schema_version=="2.0" else P21_VERSION if schema_version=="2.1" else "")
    ...
    digest=canonical_policy_sha256(policy);expected_sha=P1_SHA if schema_version=="1.0" else P2_SHA if schema_version=="2.0" else P21_SHA
