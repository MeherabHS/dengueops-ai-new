...
class ActiveModelTests(unittest.TestCase):
    def test_exact_profile_fallback(self):
        with tempfile.TemporaryDirectory() as directory:
            authority=resolve_active_model(ROOT,Path(directory));self.assertEqual(authority["authoritySource"],"historical_profile_fallback_pending_explicit_bootstrap");self.assertTrue(authority["bootstrapRequired"]);self.assertEqual(authority["parameterSha256"],PARAMETER_SHA)
    ...
    def test_history_without_pointer_fails_closed(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaises(ActiveModelError):resolve_active_model(ROOT,Path(directory))
    ...
    def test_canonical_quick_policy_mismatch_fails_closed(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaises(ActiveModelError):resolve_active_model(repository,runtime)
