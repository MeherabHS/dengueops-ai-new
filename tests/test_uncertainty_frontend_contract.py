...
class UncertaintyFrontendContractTests(unittest.TestCase):
    ...
    def test_redesigned_routes_keep_range_and_planning_separate(self):
        ...
        self.assertIn("baseline, constrained, and severe scenarios", preparedness)
        self.assertIn("operational use prohibited", preparedness)
        self.assertIn("start model assessment", forecast)
        self.assertIn("run forecast with assessed dataset", forecast)
