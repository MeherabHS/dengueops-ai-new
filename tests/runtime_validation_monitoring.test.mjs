...
test("Validation page exposes read-only lifecycle-aware monitoring",async()=>{
  ...
  assert.match(panel,/quick_forecast_p1/);assert.match(panel,/quick_forecast_p2/);assert.match(panel,/approved_forecast_p1/);assert.match(panel,/approved_forecast_p2/);
  assert.match(panel,/does not classify model health/);assert.match(panel,/Performance monitoring awaiting verified outcomes/);assert.match(panel,/not an invalid-policy/);assert.match(panel,/Unknown source rejected/);
  assert.match(panel,/<details[\s\S]*Technical evidence[\s\S]*summary\.policyVersion/);
