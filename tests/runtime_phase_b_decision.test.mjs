...
const root=process.cwd(),secret="phase-b-decision-secret-123";
...
test("current p2-v3 decision route functionally governs winner and eligible override",{timeout:180000},async()=>{
 const temporary=await mkdtemp(path.join(tmpdir(),"dengueops-phase-b-decision-")),built=spawnPythonSync(["-m","tests.runtime_phase_b_fixture_cli",path.join(temporary,"base")],{cwd:root,timeout:150000});assert.equal(built.status,0,built.stderr);const fixture=JSON.parse(built.stdout.trim().split(/\r?\n/).at(-1));
 try{
