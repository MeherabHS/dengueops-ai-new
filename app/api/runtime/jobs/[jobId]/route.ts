...
export async function GET(_request: Request, context: RouteContext<"/api/runtime/jobs/[jobId]">): Promise<Response> {
  ...
  try {
    ...
    } else if (job.jobKind === "operational_preparedness") {
      response = { ok: true, jobKind: "operational_preparedness", jobId: job.jobId, preparednessId: job.preparednessId, workflowMode: job.workflowMode, status: job.status, progress: job.progress, createdAt: job.createdAt, startedAt: job.startedAt, updatedAt: job.updatedAt, completedAt: job.completedAt, retryable: job.error?.retryable ?? false, error: job.error, committedPreparednessId: job.committedPreparednessId };
    } else {
      response = {
