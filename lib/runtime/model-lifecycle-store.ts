...
import {modelLabel} from "../status-labels";
...
export async function readModelLifecycleSummary(repositoryRoot:string,runtimeRoot:string,deploymentId="dhaka_south"){
 const current=await resolveActiveModel(repositoryRoot,runtimeRoot,deploymentId),authority={...current,modelLabel:modelLabel(current.modelId),authorityLabel:"Verified committed assignment" as const},historyRoot=path.join(runtimeRoot,"model-lifecycle"),history:Array<{lifecycleDecisionId:string;action:LifecycleAction;createdAt:string;modelIdentityChanged:boolean;assignmentProduced:boolean}>=[];
 ...
 history.sort((a,b)=>b.createdAt.localeCompare(a.createdAt));return{authority,history:history.slice(0,20),rollbackAvailable:false,humanGoverned:true,automaticActionAllowed:false,materialWorseningStatus:"not_governed" as const,statisticalSufficiencyStatus:"not_governed" as const,modelQualificationStatus:"not_governed" as const};
}
