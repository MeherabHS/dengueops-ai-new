...
const P1="0121c2fad28b7b8e9080df52698593d1cab677febf4fa668e11f6f19541fb249",P2="c73461e211e334733309232806fa2d41c2e5fdce7aa5e096d065e13e7525eaab",P21="5c3e1f7f14ab6a0a2fbc28639411a0269224b6f71746a315b9c6e159a6eacca6",P22="cc3b533b7cfc16b841401b7a60f6308632526f58d5685578d7f368dec96a09d9";
...
function policy(version:unknown,digest:unknown,schema:unknown){return(schema==="1.0"&&version==="p1.4g-v1"&&digest===P1)||(schema==="2.0"&&version==="p2-v1"&&digest===P2)||(schema==="2.1"&&version==="p2-v2"&&digest===P21)||(schema==="2.1"&&version==="p2-v3"&&digest===P22);}
...
export async function readVerifiedForecastOutcome(runtimeRoot:string,outcomeId:string){
  ...
  try{
    ...
    if(version==="2.0"||version==="2.1"){
      const allowed=version==="2.0"?["quick_forecast_p1","approved_forecast_p1","approved_forecast_p2"]:["quick_forecast_p1","quick_forecast_p2","approved_forecast_p1","approved_forecast_p2"],expectedPolicy=version==="2.0"?"p2-v1":c.policyVersion,expectedPolicySha=version==="2.0"?P2:c.policySha256;
      const sourceFamily=String(e.sourceFamily),monitoringPolicy=e.monitoringPolicy as Record<string,unknown>;
      if(!allowed.includes(sourceFamily)||e.sourceForecastRunId!==runId||o.sourceFamily!==e.sourceFamily||monitoringPolicy.policyVersion!==expectedPolicy||monitoringPolicy.policySha256!==expectedPolicySha||c.sourceFamily!==e.sourceFamily||c.profileModified!==false||c.authorizationModified!==false)return integrity();
      const approved=sourceFamily.startsWith("approved_forecast"),expectedWorkflow=approved?"approved_assessment_forecast":"quick_forecast",expectedSchema=sourceFamily==="quick_forecast_p2"?"2.1":sourceFamily==="approved_forecast_p2"?"2.0":"1.0";if(sourceCommitValue.workflowMode!==expectedWorkflow||sourceCommitValue.schemaVersion!==expectedSchema)return integrity();
      const sourceHashes=sourceCommitValue.artifactHashes as Record<string,string>,evidence=e.sourceEvidence as Record<string,unknown>;const required=approved?["forecast_output.json","forecast_uncertainty.json","model_card.json"]:["forecast_output.json","forecast_uncertainty.json","forecast_calibration.json","model_card.json"];
...
export async function readVerifiedMonitoringSummary(runtimeRoot:string,deploymentId:string){
  ...
  try{
    const pointer=await json(latestPath);const outcomeId=String(pointer.value.outcomeId??"");const verified=await readVerifiedForecastOutcome(runtimeRoot,outcomeId);const p=forecastOutcomePaths(runtimeRoot,outcomeId);const[commitBytes,summaryBytes]=await Promise.all([readFile(p.commit),readFile(p.summary)]);const v=pointer.value,s=verified.summary as Record<string,unknown>;
