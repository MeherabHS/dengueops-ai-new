...
export type Phase21ForecastOutcomePolicy=Common&{schema_version:"2.1";policy_version:"p2-v2";source_families:Record<ForecastSourceFamily,Record<string,unknown>>;candidate_registry_sha256:string;feature_order_sha256:string;target_column:"target_cases_next_2w";forecast_horizon_weeks:2};
export type ForecastOutcomePolicy=Phase1ForecastOutcomePolicy|Phase2ForecastOutcomePolicy|Phase21ForecastOutcomePolicy;
export type ForecastOutcomePolicyIdentity={schemaVersion:"1.0";policyVersion:"p1.4g-v1";policySha256:typeof P1_SHA}|{schemaVersion:"2.0";policyVersion:"p2-v1";policySha256:typeof P2_SHA}|{schemaVersion:"2.1";policyVersion:"p2-v2";policySha256:typeof P21_SHA};
...
export async function loadForecastOutcomePolicy(repositoryRoot:string,identity:ForecastOutcomePolicyIdentity={schemaVersion:"2.1",policyVersion:"p2-v2",policySha256:P21_SHA}):Promise<ForecastOutcomePolicy>{
  ...
  if(!phase1&&!phase2&&!phase21)return invalid();
  const filename=phase1?"forecast_outcome_policy_p1.4g-v1.json":phase2?"forecast_outcome_policy_p2-v1.json":"forecast_outcome_policy.json";
  try{
    ...
    const expectedSha=phase1?P1_SHA:phase2?P2_SHA:P21_SHA,expectedVersion=phase1?"p1.4g-v1":phase2?"p2-v1":"p2-v2",expectedSchema=phase1?"1.0":phase2?"2.0":"2.1";
    ...
    if(phase21){const value=policy as Phase21ForecastOutcomePolicy;if(Object.keys(value.source_families??{}).sort().join("|")!=="approved_forecast_p1|approved_forecast_p2|quick_forecast_p1|quick_forecast_p2")return invalid();}
    return policy;
