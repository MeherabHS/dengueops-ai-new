...
export type ForecastOutcomeJobRecord=(ForecastOutcomeJobCommon&{schemaVersion:"1.0";policyVersion:"p1.4g-v1"})|(ForecastOutcomeJobCommon&{schemaVersion:"2.0";policyVersion:"p2-v1"})|(ForecastOutcomeJobCommon&{schemaVersion:"2.1";policyVersion:"p2-v2"|"p2-v3"});
...
export interface MonitoringSummary {
  schemaVersion:"1.0"|"2.0"|"2.1";deploymentId:"dhaka_south";policyId:"RUNTIME.FORECAST_OUTCOME.MONITORING";policyVersion:"p1.4g-v1"|"p2-v1"|"p2-v2"|"p2-v3";policySha256:string;
...
export interface DegradationEvidenceJobRecord{schemaVersion:"2.0";jobKind:"degradation_evidence";jobId:string;evidenceId:string;deploymentId:"dhaka_south";geography:{level:"city";id:"BGD-DHAKA-SOUTH";name:"Dhaka South"};workflowMode:"degradation_evidence";policyId:"RUNTIME.MODEL_DEGRADATION.EVIDENCE";policyVersion:"p2-v3";policySha256:string;expectedMonitoringLatestSha256:string;expectedMonitoringSummarySha256:string;expectedIncludedOutcomeSetSha256:string;evidenceOnlyAcknowledged:true;status:RuntimeJobStatus;progress:string;createdAt:string;claimedAt:string|null;startedAt:string|null;updatedAt:string;completedAt:string|null;heartbeatAt:string|null;workerId:string|null;processId:number|null;timeoutSeconds:number;retryCount:0;error:{code:string;message:string;retryable:boolean}|null;committedEvidenceId:string|null}
...
export interface ModelDegradationCohort{cohortId:string;outcomeSetSha256:string;identity:{deploymentId:"dhaka_south";sourceFamily:ForecastOutcomeSourceFamily;modelId:string;modelFamily:string;parameterSha256:string;preprocessingIdentity:string|null;candidateRegistrySha256:string;featureOrderSha256:string;sourcePolicy:{policyId:string;policyVersion:string;policySha256:string};forecastHorizonWeeks:2;forecastPresentationMode:"historical"|"point_only"|"point_and_interval";calibrationStatus:string};outcomeCount:number;actualPopulation:{mae:number;rmse:number;signedBias:number;absoluteBias:number;mpe:number|null;mape:number|null;percentageEligibleCount:number;rangeEligibleCount:number;empiricalRangeEvaluatedCount:number;empiricalRangeCoveredCount:number;empiricalCoverage:number|null;lowerMissCount:number;upperMissCount:number};trainingContext:{trainingRowCounts:number[];trainingPeriods:string[];variationRecorded:boolean};assignmentProvenance:Array<{assignmentId:string;assignmentCommitSha256:string;assignmentAction:"assign_selected_model";authoritySnapshotSha256:string;lifecyclePolicy:{policyId:string;policyVersion:string;policySha256:string}}>;assessmentReferenceStatus:"computable_descriptive_evidence"|"not_applicable_no_assessment_reference";assessmentReferences:ModelDegradationAssessmentReference[];monitoringWindow:{status:"window_size_not_governed";windowOutcomeCount:null;metricsCalculated:false;statisticalSufficiencyStatus:"not_governed"};warnings:string[]}
export interface ModelDegradationEvidence{schemaVersion:"2.0";evidenceId:string;deploymentId:"dhaka_south";degradationPolicy:{policyId:"RUNTIME.MODEL_DEGRADATION.EVIDENCE";policyVersion:"p2-v2";policySha256:string};monitoringPolicy:{policyId:"RUNTIME.FORECAST_OUTCOME.MONITORING";policyVersion:"p2-v2";policySha256:string;schemaVersion:"2.1"};monitoringInput:{latestSnapshotPath:"artifacts/monitoring_latest_snapshot.json";latestSnapshotSha256:string;summaryPath:string;summarySha256:string;includedOutcomeSetSha256:string;verifiedOutcomeCount:number};evidenceStatus:"evidence_only";degradationThresholdStatus:"not_governed";degradationThresholds:null;statisticalSufficiencyStatus:"not_governed";materialWorseningClassificationAllowed:false;materialWorseningStatus:"not_governed";lifecycleRecommendationAllowed:false;lifecycleActionProduced:false;lifecycleActionStatus:"prohibited_not_generated";cohorts:ModelDegradationCohort[];includedCohortSetSha256:string;generatedAt:string;limitations:string[]}
export interface ModelDegradationSummary{schemaVersion:"2.0";evidenceId:string;policyId:"RUNTIME.MODEL_DEGRADATION.EVIDENCE";policyVersion:"p2-v2";policySha256:string;verifiedOutcomeCount:number;cohortCount:number;assessmentReferenceDimensionCount:number;computableDescriptiveDimensionCount:number;windowSizeNotGovernedDimensionCount:number;percentageUnavailableDimensionCount:number;rangeUnavailableDimensionCount:number;sourceFamilyCounts:Record<string,number>;modelCounts:Record<string,number>;policyCounts:Record<string,number>;latestTargetPeriod:string;includedCohortSetSha256:string;includedOutcomeSetSha256:string;evidenceStatus:"evidence_only";degradationThresholdStatus:"not_governed";statisticalSufficiencyStatus:"not_governed";materialWorseningStatus:"not_governed";lifecycleActionStatus:"prohibited_not_generated"}
...
export type CurrentLifecycleAuthority=CurrentActiveModelAuthority&{
  modelLabel:string;
  authorityLabel:"Verified committed assignment";
};
...
export type JobStatusResponse =
  ...
  | ({ ok: true; jobKind: "dataset_assessment"; jobId: string; assessmentId: string; status: RuntimeJobStatus; progress: string; createdAt: string; startedAt: string | null; updatedAt: string; completedAt: string | null; retryable: boolean; error: RuntimeJobRecord["error"]; committedAssessmentId: string | null; verifiedCandidateCount?: number })
...
export type LatestDashboardResponse =
  | { ok: true; sourceType: "uploaded"; runId: string; dashboard: import("@/lib/dashboard-view-model").OverviewViewModel }
  | RuntimeErrorResponse;
