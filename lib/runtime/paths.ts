...
export function currentModelDegradationLatestPaths(runtimeRoot:string,deploymentId:string){const historical=modelDegradationLatestPaths(runtimeRoot,deploymentId);return{...historical,latest:assertContained(historical.root,path.join(historical.root,"latest_p2-v2.json"))};}
