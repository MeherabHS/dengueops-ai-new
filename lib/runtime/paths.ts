...
export function modelDegradationPaths(runtimeRoot:string,evidenceId:string){const collections=runtimeCollectionPaths(runtimeRoot);const staging=uuidPath(collections.degradationStaging,evidenceId,"evidence");const committed=uuidPath(collections.degradationEvidence,evidenceId,"evidence");return{staging,committed,evidence:assertContained(committed,path.join(committed,"artifacts","degradation_evidence.json")),summary:assertContained(committed,path.join(committed,"artifacts","degradation_summary.json")),monitoringLatestSnapshot:assertContained(committed,path.join(committed,"artifacts","monitoring_latest_snapshot.json")),commit:assertContained(committed,path.join(committed,"metadata","commit.json"))};}
...
export function currentModelDegradationLatestPaths(runtimeRoot:string,deploymentId:string){const historical=modelDegradationLatestPaths(runtimeRoot,deploymentId);return{...historical,latest:assertContained(historical.root,path.join(historical.root,"latest_p2-v3.json"))};}
