...
export default function RuntimeAssessmentWorkflow({ assessmentId }: { assessmentId: string }) {
  ...
  return <section className="space-y-5" aria-labelledby="uploaded-assessment-heading">
    ...
    {assessment.scientificValidation ? <div className="rounded-xl border border-success/25 bg-success/10 p-5"><h3 className="font-semibold text-ink">Temporal validation</h3><dl className="mt-3 grid gap-2 text-sm text-ink-muted md:grid-cols-3"><div><dt className="font-medium text-ink">Temporal validation</dt><dd>Passed</dd></div><div><dt className="font-medium text-ink">Rolling-origin assessment</dt><dd>{assessment.scientificValidation.foldCountCompleted} / {assessment.scientificValidation.foldCountRequired} folds</dd></div><div><dt className="font-medium text-ink">Leakage checks</dt><dd>Passed</dd></div><div><dt className="font-medium text-ink">Dataset revision basis</dt><dd>Latest retrospective snapshot</dd></div></dl><details className="mt-3 text-xs"><summary className="cursor-pointer font-semibold text-ink">Technical evidence</summary><p className="mt-2 font-mono">Target-derived purge: {assessment.scientificValidation.purgeGapWeeks} weeks · Preprocessing: {assessment.scientificValidation.preprocessingScope} · Policy: {assessment.scientificValidation.featureAvailabilityPolicy.policyVersion}</p><p className="mt-1 text-warning">True historical data vintages are unavailable. Qualification is workflow execution evidence, not an untouched scientific holdout.</p></details></div> : null}
