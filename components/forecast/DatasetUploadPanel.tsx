...
export default function DatasetUploadPanel({ kind, preview, onChange, onRemove, workflow = "assessment" }: { kind: "dengue" | "climate"; preview?: LocalFilePreview; onChange: (preview: LocalFilePreview) => void; onRemove: () => void; workflow?: "assessment" | "operational" }) {
  const input = useRef<HTMLInputElement>(null); const [dragging,setDragging] = useState(false); const [reading, setReading] = useState(false);
  const label = kind === "dengue" ? "Dengue case data" : "Climate data";
  const accept = async (file?: File) => {
    if (!file || reading) return;
    setReading(true);
    try {
      onChange(await parse(file, kind));
    } finally {
      setReading(false);
    }
  };
  return <section className="rounded-xl border border-border-subtle bg-surface p-5 shadow-sm" aria-labelledby={`${kind}-upload-title`}>
    ...
    <label className={`flex min-h-40 cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed p-5 text-center focus-within:ring-2 focus-within:ring-focus ${dragging ? "border-accent bg-accent/10" : "border-border-subtle bg-surface-muted"}`} onDragOver={e => {e.preventDefault(); setDragging(true)}} onDragLeave={() => setDragging(false)} onDrop={e => {e.preventDefault();setDragging(false);void accept(e.dataTransfer.files[0])}}>
      ...
      <input ref={input} className="sr-only" type="file" accept=".csv,text/csv" disabled={reading} aria-label={`Choose ${label} CSV`} onChange={e => void accept(e.target.files?.[0])} />
    </label>
    {reading ? <div className="mt-3"><AsyncStatusIndicator label={workflow === "assessment" ? "Reading assessment data…" : "Reading forecast data…"} delayedAfterSeconds={10} /></div> : null}
